...

{
  ...
  for (i = 1; i <= nx; i++)
    for (j = 1; j <= ny; j++) {
      ...
      */
      double r = u_RGB[0][i][j];
      double g = u_RGB[1][i][j];
      double b = u_RGB[2][i][j];

      u_YCbCr[0][i][j] = 0.299 * r + 0.587 * g + 0.114 * b;
      u_YCbCr[1][i][j] = 127.5 + -0.1687 * r - 0.3313 * g + 0.5 * b;
      u_YCbCr[2][i][j] = 127.5 + 0.5 * r - 0.4187 * g - 0.0813 * b;
    }
